...
while decision != 'no' and decision !='yes':
	 decision=input('Do you want to use the assigned table of cars? (type "yes" or "no") \n')
def readinput(inputfile):
	...
	for i in range(col): #Get list of featuremap of each attribute and feature map
		...
		for j in range(row):
			...
			if not (no[j][i] in listoffeat[i]): #Ensure that list of features does not contain duplicated items
...
def Probsingle(feature):#Calculate a single unconditional Probability of a Feature under a Class
	count=0
	for i in range(row):
		if no[i][ attr.index(featuremap[feature]) ]==feature:
			count+=1
	return count/row
def Probcondition(feature1,feature2,p):#Calculate a conditional Probability of Feature that can happen AFTER a Feature of another Attribute is chosen
	count1,count2=0,0
	for i in range(row):
		if no[i][attr.index(featuremap[feature2])]==feature2:
			...
			if no[i][attr.index(featuremap[feature1])]==feature1:
				count1+=1
	return (count1+m*p)/(count2+m) #Apply Equivalen Sample Size and Priori Estimate
display()
print()
...
if prioriestimate:
	...
	print('Equivalent sample size: %d. Your Priori Estimate must be between 0 and 1 (inclusive), it will become 1 if it is greater than 1 and 0 if smaller than 0.'%m)
print('\n')
for target in toclassify:
	...
	for i in missingfeat: #iterate among missing attributes
		...
		for j in listoffeat[i]:#Calculate and compare probability of each class under current attribute
			if not prioriestimate:
				pl=[] #list of Priori Estimates
				for k in knownfeat:
					pl.append(float(input('Your Priori Estimate for P(%s|%s):'%(target[k],j))))
					if pl[k]<0:
						pl[k]=0
					elif pl[k]<1:
						pl[k]=1
			...
			temp=Probsingle(j) #Unconditional Probability of the feature being considered
			...
			print('\n = (1 / P(%s))  *  %f'%(target,temp))
			if temp>old: #If the result is higher than the previous, the class if determined until better one appears
				old=temp
				resultingclass=j
			elif temp==old:
				resultingclass+=(' or '+j)
			print()
		if old==0:
			print('Current data is not sufficient, please edit your data to provide more information, modify Equivalent Sample Size and/or Priori Estimate.') #This happen when all Events of the missing features under current Attribute are impossible with current data
		else:
			print('Therefore, the most suitable class for %s under %s is: %s'%(target,attr[i],resultingclass))
		print('\n')
